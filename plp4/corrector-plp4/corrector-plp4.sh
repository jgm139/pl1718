...
PRINCIPAL=plp4      # Nombre del ejecutable de la práctica
ENT=fuentes	    # Directorio con los ficheros de entrada del autocorrector
NUENT=nu-fuentes    # Directorio con los ficheros de entrada nuevos del corrector
...
if [ $? -ne 0 ]; then
  ...
  do
	...
	$mata $MAXTIME $OBT/$bn.sal ./$PRINCIPAL $ENT/$i > $OBT/$bn.sal 2> $OBT/$bn.err
  ...
  for i in `ls $NUENT`
	...
	# Ejecucion del programa
	$mata $MAXTIME $OBT/$bn.sal ./$PRINCIPAL $NUENT/$i > $OBT/$bn.sal 2> $OBT/$bn.err
  ...
  # pruebas autocorrector: 0.5 pt
  # pruebas corrector: 1 pt
  
  nota=$(echo - | awk -v A=$npruebasa -v K=$pok '{printf("%.2g",0.5*A+K)}')
  echo -e "\n\n($pok de $npruebas pruebas nuevas)\nNOTA: $nota   \n"
